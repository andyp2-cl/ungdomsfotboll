...
export function CupsTabContent({ cups, onActivitySelect, allActivities = [] }: CupsTabContentProps) {
  // Function to count matches in a cup
  const getMatchCountForCup = (cupId: string): number => {
    return allActivities.filter(activity => 
      activity.type === "match" && activity.cupId === cupId
    ).length;
  };
    ...
    <>
      {cups.length > 0 ? (
        <div className="space-y-4">
          {cups.map(cup => {
            const matchCount = getMatchCountForCup(cup.id);
